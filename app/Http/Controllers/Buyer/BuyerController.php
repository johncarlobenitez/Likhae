...
class BuyerController extends Controller
{
    ...
    {
        $shop = \App\Models\Seller\Seller::with('owner')->where('status', 'approved')->where(function ($query) use ($seller) {
    ...
    public function deleteAddress(Request $request, Address $address): RedirectResponse
    {
        ...
        abort_if(\App\Models\Seller\Seller::where('pickup_address_id', $address->id)->whereIn('status', ['pending', 'approved'])->exists(), 409, 'Reassign the seller pickup address before deleting it.');
