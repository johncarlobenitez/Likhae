...
class CheckoutController extends Controller
{
    ...
    public function show(Request $request): View|RedirectResponse
    {
        ...
        if ($request->filled('buy')) {
            $product = \App\Models\Seller\Product::query()->where('is_active', true)->where('slug', (string) $request->input('buy'))->first();
            $variantId = $request->input('product_variant_id');
            $variant = $variantId ? \App\Models\Seller\ProductVariant::query()->with(['product.seller', 'product.images'])->where('product_id', $product?->id)->whereKey((int) $variantId)->first() : null;
        ...
        if (is_array($buyNow) && ! empty($buyNow['product_variant_id'])) {
            $variant = \App\Models\Seller\ProductVariant::query()->with(['product.seller', 'product.images'])->whereKey((int) $buyNow['product_variant_id'])->first();
