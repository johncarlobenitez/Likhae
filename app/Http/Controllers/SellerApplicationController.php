...
class SellerApplicationController extends Controller
{
    public function create(Request $request): View|RedirectResponse
    {
        ...
        return view('onboarding.seller-apply', ['addresses' => $request->user()->addresses()->latest()->get(), 'seller' => $seller]);
    }
    ...
    public function entry(Request $request): View|RedirectResponse
    {
        $seller = $request->user()->sellers()->latest()->first();
        if (! $seller) {
            return view('onboarding.status', ['type' => 'shop', 'record' => null, 'applyRoute' => route('sell.create')]);
        }
        if ($seller->status === 'approved' && $request->user()->hasRole('seller')) {
            return redirect()->route('seller.dashboard');
        }

        return view('onboarding.status', ['type' => 'shop', 'record' => $seller, 'applyRoute' => route('sell.create')]);
    }
}
