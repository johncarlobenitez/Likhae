...
<div class="ad-page">
    ...
    @if($rider)
        <div class="ad-card ad-card-body" style="margin-bottom:16px;"><h2>Rider / Courier Information</h2><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:16px;margin-top:16px;"><div><small>Vehicle</small><strong style="display:block;">{{ str($rider->vehicle_type)->headline() }}</strong></div><div><small>Plate Number</small><strong style="display:block;">{{ $rider->plate_number }}</strong></div><div><small>Driver's License Number</small><strong style="display:block;">{{ $rider->drivers_license_number }}</strong></div><div><small>Sorting Center</small><strong style="display:block;">{{ $rider->targetLogisticsCenter?->business_name ?? 'Not configured' }}</strong></div></div></div>
    @endif
