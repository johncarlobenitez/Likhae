...
<div class="lk-page-narrow">
    ...
    @elseif(in_array($mode, ['review','return'], true))
        @if($selectedOrder)
            <div class="lk-page-title"><div><span class="lk-kicker">Order #{{ data_get($selectedOrder,'id') }}</span><h1>{{ $mode === 'review' ? 'Review Product' : 'Return / Refund Request' }}</h1><p>{{ $mode === 'review' ? 'Share an honest review to help other buyers.' : 'Provide the reason and evidence for the seller to review.' }}</p></div><a class="lk-btn lk-btn-light" href="{{ route('buyer.orders') }}">Back to Orders</a></div>
            <form method="POST" action="{{ $mode === 'review' ? route('buyer.orders.review.store', ['id' => data_get($selectedOrder,'id')]) : route('buyer.orders.return.store', ['id' => data_get($selectedOrder,'id')]) }}" class="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm sm:p-6" enctype="multipart/form-data">
                ...
                @else
                    <div class="mt-5 grid gap-4 sm:grid-cols-2"><label class="text-xs font-semibold text-stone-700">Request type<select required name="request_type" class="mt-2 block w-full rounded-xl border border-stone-300 px-3 py-2.5 text-sm outline-none focus:border-red-800"><option value="">Choose an option</option><option>Return and refund</option><option>Refund only</option></select></label><label class="text-xs font-semibold text-stone-700">Reason<select required name="reason" class="mt-2 block w-full rounded-xl border border-stone-300 px-3 py-2.5 text-sm outline-none focus:border-red-800"><option value="">Choose a reason</option><option>Damaged item</option><option>Defective item</option><option>Wrong product</option><option>Wrong variation</option><option>Missing item</option><option>Missing parts</option><option>Significantly different from description</option><option>Other</option></select></label></div>
                    <label class="mt-4 block text-xs font-semibold text-stone-700">Explain what happened<textarea required minlength="20" name="details" rows="5" class="mt-2 block w-full rounded-xl border border-stone-300 p-3 text-sm outline-none focus:border-red-800" placeholder="Give the seller enough detail to review this request."></textarea></label>
                                        <div class="mt-4 rounded-xl bg-amber-50 p-4 text-xs leading-5 text-amber-900"><strong>Seller review:</strong> The seller reviews your explanation and evidence, then responds to the return or refund request.</div>
                    <button class="lk-btn lk-btn-red mt-5" type="submit">Submit Request</button>
                @endif
            </form>
        @else
    ...
    @elseif($mode === 'show')
        @if($selectedOrder)
            ...
            <div class="grid gap-5 lg:grid-cols-[1fr_340px]">
                ...
                <aside class="space-y-5"><section class="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm"><h2 class="text-base font-bold text-stone-900">Delivery details</h2><strong class="mt-3 block text-xs">{{ data_get($selectedOrder,'buyer_name','Buyer') }}</strong>@if(data_get($selectedOrder,'buyer_contact'))<p class="mt-1 text-xs text-stone-500">{{ data_get($selectedOrder,'buyer_contact') }}</p>@endif<p class="mt-2 whitespace-pre-line text-xs leading-5 text-stone-500">{{ data_get($selectedOrder,'shipping_address','No delivery address recorded.') }}</p>@if(data_get($selectedOrder,'tracking'))<p class="mt-3 text-xs font-semibold text-stone-700">Tracking: {{ data_get($selectedOrder,'tracking') }}</p>@endif</section><section class="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm"><h2 class="text-base font-bold text-stone-900">Payment summary</h2><dl class="mt-3 space-y-2 text-xs"><div class="flex justify-between"><dt class="text-stone-500">Order total</dt><dd>₱{{ number_format((float) data_get($selectedOrder,'total'),2) }}</dd></div><div class="flex justify-between border-t border-stone-100 pt-3 font-bold"><dt>Status</dt><dd class="text-red-900">{{ data_get($selectedOrder,'status_label') }}</dd></div></dl></section></aside>
            </div>
