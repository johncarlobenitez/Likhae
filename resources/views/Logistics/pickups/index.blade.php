        ...
        <table class="min-w-full text-left text-sm">
            ...
            @forelse($pickupRows as $row)
                <tr>
                    ...
                    <td class="p-4"><div class="font-semibold text-ink">{{ $row['parcel']['seller'] }}</div><div class="max-w-sm text-muted">{{ collect($row['delivery']->sellerOrder?->seller?->pickupAddress?->only(['line1', 'barangay', 'city', 'province']))->filter()->implode(', ') ?: 'Seller address not recorded' }}</div></td>
                    <td class="p-4 font-semibold">{{ Str::headline($row['delivery']->status) }}</td>
                    <td class="p-4">
                        @if(in_array($row['delivery']->status, ['unassigned', 'assigned'], true))
                        ...
                        @else
                            <span class="text-muted">{{ $row['delivery']->rider?->user?->name ?: 'Unassigned' }}</span>
                        @endif
                    </td>
                </tr>
            @empty
