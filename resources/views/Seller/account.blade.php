        ...
        <main class="sl-account-content">
            @if($currentTab === 'business')
                <form class="sl-card sl-form-section" method="POST" action="{{ route('seller.account.business') }}">@csrf @method('PUT')<header class="sl-card-head"><div><h2>Business Information</h2><p>Legal information used for marketplace compliance.</p></div></header><div class="sl-form-grid">
                    <label class="sl-field"><span>Registered business name</span><input name="business_name" value="{{ old('business_name',$storeProfile->business_name ?? '') }}"></label><label class="sl-field"><span>Business type</span><input name="business_type" value="{{ old('business_type',$storeProfile->business_type ?? '') }}"></label><label class="sl-field"><span>DTI/SEC registration number</span><input name="dti_sec_number" value="{{ old('dti_sec_number',$storeProfile->dti_sec_number ?? '') }}"></label><label class="sl-field"><span>TIN</span><input name="tin" value="{{ old('tin',$storeProfile->tin ?? '') }}"></label><label class="sl-field"><span>Contact number</span><input name="contact_number" value="{{ old('contact_number',$seller->contact_number) }}"></label><label class="sl-field"><span>Province</span><input name="province" value="{{ old('province',$storeProfile->province ?? '') }}"></label><label class="sl-field"><span>Municipality</span><input name="municipality" value="{{ old('municipality',$storeProfile->municipality ?? '') }}"></label><label class="sl-field"><span>Barangay</span><input name="barangay" value="{{ old('barangay',$storeProfile->barangay ?? '') }}"></label><label class="sl-field"><span>House number</span><input name="house_number" value="{{ old('house_number',$storeProfile->house_number ?? '') }}"></label><label class="sl-field"><span>Street</span><input name="street" value="{{ old('street',$storeProfile->street ?? '') }}"></label>
