    ...
    @elseif (!$isLogistics && $currentMode === 'show')
        @if($order)
            ...
            <section class="sl-card"><header class="sl-card-head"><div><h2>Fulfillment Actions</h2><p>Available actions depend on the current status.</p></div></header><div class="sl-action-panel"><span class="sl-action-panel-icon">✓</span><div><strong>{{ $order['status'] }}</strong><p>Use the valid next action to move the order through fulfillment.</p></div>
                @if($order['status_key']==='placed')<form method="POST" action="{{ route('seller.orders.status',$order['db_id']) }}">@csrf @method('PATCH')<input type="hidden" name="status" value="accepted"><button class="sl-btn sl-btn-primary" type="submit">Accept Order</button></form>@endif
                @if($order['status_key']==='confirmed')<form method="POST" action="{{ route('seller.orders.status',$order['db_id']) }}">@csrf @method('PATCH')<input type="hidden" name="status" value="packed"><button class="sl-btn sl-btn-primary" type="submit">Packed</button></form>@endif
                @if($order['status_key']==='preparing')<form method="POST" action="{{ route('seller.orders.status',$order['db_id']) }}">@csrf @method('PATCH')<input type="hidden" name="status" value="ready_to_ship"><button class="sl-btn sl-btn-primary" type="submit">Ready to Ship</button></form>@endif
