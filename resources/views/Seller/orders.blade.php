    ...
    @elseif (!$isLogistics && $currentMode === 'show')
        @if($order)
            ...
            <section class="sl-card"><header class="sl-card-head"><div><h2>Fulfillment Actions</h2><p>Available actions depend on the current status.</p></div></header><div class="sl-action-panel"><span class="sl-action-panel-icon">✓</span><div><strong>{{ $order['status'] }}</strong><p>Use the valid next action to move the order through fulfillment.</p></div>
                @if($order['status_key']==='placed')<form method="POST" action="{{ route('seller.orders.status',$order['db_id']) }}">@csrf @method('PATCH')<input type="hidden" name="status" value="confirmed"><button class="sl-btn sl-btn-primary" type="submit">Accept Order</button></form>@endif
                @if($order['status_key']==='confirmed')<form method="POST" action="{{ route('seller.orders.status',$order['db_id']) }}">@csrf @method('PATCH')<input type="hidden" name="status" value="preparing"><button class="sl-btn sl-btn-primary" type="submit">Start Preparation</button></form>@endif
                @if($order['status_key']==='preparing')<a target="_blank" class="sl-btn sl-btn-ghost" href="{{ route('seller.orders.waybill',$order['db_id']) }}">View Waybill</a><form method="POST" action="{{ route('seller.orders.status',$order['db_id']) }}">@csrf @method('PATCH')<input type="hidden" name="status" value="ready_for_pickup"><button class="sl-btn sl-btn-primary" type="submit">Mark Ready for Pickup</button></form>@endif
                @if($order['status_key']==='ready-for-pickup')<a target="_blank" class="sl-btn sl-btn-ghost" href="{{ route('seller.orders.waybill',$order['db_id']) }}">Print Waybill</a><a href="{{ route('seller.logistics',['view'=>'couriers','order'=>$order['id']]) }}" class="sl-btn sl-btn-primary">Choose Handover</a>@endif
