            ...
            <div class="sl-editor-main">
                <section class="sl-card sl-form-section">
                    ...
                    <div class="sl-form-grid">
                        ...
                        <label class="sl-field"><span>Category</span><select name="category_id"><option value="">Uncategorized</option>@foreach($categories as $category)<option value="{{ $category->id }}" @selected((string) old('category_id', data_get($product, 'category_id')) === (string) $category->id)>{{ $category->name }}</option>@endforeach</select></label>
