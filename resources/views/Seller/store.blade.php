...
<div class="sl-page">
    ...
    @else
        <form class="sl-store-layout" method="POST" enctype="multipart/form-data" action="{{ route('seller.store.update') }}">@csrf @method('PUT')
            <aside class="sl-card sl-store-preview">@if($bannerUrl)<img class="sl-store-cover" src="{{ $bannerUrl }}" alt="{{ $shopName }} banner">@endif<div class="sl-shop-logo">@if($avatarUrl)<img src="{{ $avatarUrl }}" alt="{{ $shopName }} avatar">@else{{ $shopInitials }}@endif</div><h2>{{ $shopName }}</h2><p>{{ $storeProfile->description ?: 'Complete your shop description.' }}</p><span class="sl-verified-store">Verified Seller</span></aside>
            <div class="sl-store-form">
                <section class="sl-card sl-form-section"><header class="sl-card-head"><div><h2>Shop Information</h2><p>This information is visible to buyers.</p></div></header><div class="sl-form-grid"><label class="sl-field sl-span-2"><span>Shop name <b>*</b></span><input name="shop_name" value="{{ old('shop_name',$storeProfile->shop_name) }}" required></label><label class="sl-field"><span>Shop profile picture</span><input type="file" name="avatar" accept="image/*"></label><label class="sl-field"><span>Shop banner image</span><input type="file" name="banner" accept="image/*"></label><label class="sl-field sl-span-2"><span>Tagline</span><input name="tagline" value="{{ old('tagline',$storeProfile->tagline) }}"></label><label class="sl-field sl-span-2"><span>Description</span><textarea name="description" rows="5">{{ old('description',$storeProfile->description) }}</textarea></label></div></section>
