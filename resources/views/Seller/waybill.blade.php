...
<style>
body{font-family:Arial,sans-serif;background:#fff;color:#231815;margin:0;padding:28px}.waybill{max-width:780px;margin:auto;border:2px solid #231815}.head,.section{padding:18px 22px;border-bottom:1px solid #231815}.head{display:flex;justify-content:space-between;align-items:flex-start}.brand{font-size:30px;font-weight:800;color:#641f19}.grid{display:grid;grid-template-columns:1fr 1fr;gap:16px}.label{font-size:11px;text-transform:uppercase;color:#777}.value{font-size:15px;font-weight:700;margin-top:4px}.items{width:100%;border-collapse:collapse}.items th,.items td{padding:10px;border-bottom:1px solid #ddd;text-align:left}.barcode{font-family:monospace;letter-spacing:5px;font-size:22px;text-align:center;padding:20px}.actions{text-align:center;margin-top:18px}@media print{.actions{display:none}body{padding:0}.waybill{border:1px solid #000}}
</style>
</head>
<body>
<div class="waybill">
    <div class="head"><div><div class="brand">LIKHAE</div><div>Seller Shipping Waybill</div></div><div><div class="label">Order</div><div class="value">#{{ $order->order_number }}</div><div class="label" style="margin-top:8px">Tracking</div><div class="value">{{ $order->delivery?->tracking_number ?: 'Pending assignment' }}</div></div></div>
