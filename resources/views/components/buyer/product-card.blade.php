...
<article class="lk-product-card {{ $compact ? 'is-compact' : '' }} {{ $guest ? 'is-guest is-view-only' : '' }}" data-product-card data-product-id="{{ $id ?? $slug }}" data-category="{{ \Illuminate\Support\Str::slug($category) }}" @unless($guest) data-price="{{ $price }}" data-rating="{{ $rating }}" data-sold="{{ $sold }}" @endunless data-search="{{ mb_strtolower($name.' '.$category.' '.$seller) }}">
    <div class="lk-product-media">
        ...
        @if(!$guest && $discount > 0)
            <span class="lk-product-discount">-{{ min(100, $discount) }}%</span>
        @endif

        @unless($guest)
            <button type="button" class="lk-product-heart" data-wishlist data-product-id="{{ $id ?? $slug }}" aria-pressed="false" aria-label="Save {{ $name }} to wishlist">
    ...
    <div class="lk-product-body">
        ...
        @unless($guest)
            <div class="lk-product-meta">
                ...
                <span class="lk-product-sold">{{ number_format($sold) }} sold</span>
            </div>
        ...
        @endunless
        <div class="lk-card-actions {{ $guest ? 'lk-card-actions--view-only' : '' }}">
            <a href="{{ $detailsUrl }}" class="lk-btn-view">{{ $guest ? 'View Details' : 'View' }}</a>
            @unless($guest)
