...
<div class="lk-page">
    ...
    <section class="lk-section" aria-labelledby="categories-heading">
        <div class="lk-section-head">
            <div>
                <span class="lk-kicker">Browse</span>
                <h2 id="categories-heading">Shop by Category</h2>
                <p>Explore products from different collections.</p>
            </div>

            <a href="{{ route('products') }}" class="lk-text-link">View All</a>
        </div>

        <div class="lk-category-grid">
            @foreach ($categories as $category)
                <a href="{{ route('products', ['category' => $category['slug']]) }}" class="lk-category-card">
    ...
    <section class="lk-section" aria-labelledby="featured-products-heading">
        <div class="lk-section-head">
            <div>
                <span class="lk-kicker">Curated</span>
                <h2 id="featured-products-heading">Featured Products</h2>
                <p>Top picks from trusted sellers.</p>
            </div>

            <a href="{{ route('products', ['sort' => 'featured']) }}" class="lk-text-link">See All</a>
        </div>
        ...
        @else
            <div class="rounded-2xl border border-dashed border-stone-300 bg-white px-6 py-12 text-center">
                <div class="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-red-50 text-red-800">
                    <svg width="27" height="27" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                        <path d="M6 8h12l1 13H5z"/>
                        <path d="M9 10V6a3 3 0 0 1 6 0v4"/>
                    </svg>
                </div>

                <h3 class="mt-4 text-sm font-semibold text-stone-900">Featured products are coming soon</h3>
                <p class="mt-1 text-xs text-stone-500">Products from trusted local sellers will appear here.</p>
            </div>
        @endif
    </section>

    <section id="flash-picks" class="lk-section scroll-mt-24" aria-labelledby="flash-picks-heading">
        <div class="lk-deal-banner">
            <div>
                <span class="lk-kicker">Limited Offers</span>
                <h2 id="flash-picks-heading">Flash Picks</h2>
                <p>Premium local finds from trusted sellers.</p>
            </div>

            <a href="{{ route('products', ['sort' => 'best-selling']) }}" class="lk-btn lk-btn-gold">View Products</a>
        </div>
    </section>

    <section class="lk-section" aria-labelledby="recommended-products-heading">
        <div class="lk-section-head">
            <div>
                <span class="lk-kicker">For You</span>
                <h2 id="recommended-products-heading">Recommended For You</h2>
                <p>Products based on popular buyer choices.</p>
            </div>

            <a href="{{ route('products', ['sort' => 'best-rated']) }}" class="lk-text-link">See All</a>
        </div>
        ...
        @else
            <div class="rounded-2xl border border-dashed border-stone-300 bg-white px-6 py-10 text-center">
                <h3 class="text-sm font-semibold text-stone-900">More recommendations will appear here</h3>
                <p class="mt-1 text-xs text-stone-500">Explore products to help us prepare recommendations for you.</p>
                <a href="{{ route('products') }}" class="mt-4 inline-flex items-center justify-center rounded-xl bg-red-900 px-4 py-2.5 text-xs font-semibold text-white transition hover:bg-red-950">Explore Products</a>
            </div>
        @endif
    </section>
</div>
