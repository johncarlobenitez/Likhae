...
@php
    ...
    $catalogUrl = fn(array $changes = []) => route('products', array_filter(array_merge(request()->only(['q','category','sort','view']), $changes), fn($value) => $value !== '' && $value !== null));
@endphp
...
<div class="lk-page">
    ...
    <div class="lk-products-layout">
        <aside class="lk-products-sidebar" aria-label="Product filters">
            <form action="{{ route('products') }}" method="GET" class="grid gap-3">
        ...
        <main class="lk-products-main">
            <div class="lk-products-toolbar">
                ...
                <div class="lk-toolbar-actions">
                    <form action="{{ route('products') }}" method="GET" class="lk-catalog-sort">@foreach(request()->except('sort') as $key => $value)<input type="hidden" name="{{ $key }}" value="{{ $value }}">@endforeach<select name="sort" onchange="this.form.submit()"><option value="latest" @selected($activeSort === 'latest')>Latest</option><option value="name" @selected($activeSort === 'name')>Name</option></select></form>
